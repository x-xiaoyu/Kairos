...
- 习惯是当天打卡，不是一次性任务。
- 回顾是独立导航页：完成数量、真实专注时长、时间盲症校准卡片、按发生顺序的时间线。

### 5.9 Kairos Agent

三种模式：本地规则、Kairos AI 后端、我的 AI（OpenAI Keychain）。

Agent 可提出但不静默执行：创建任务、顺延、改时长、改优先级、完成、重新规划。涉及数据变化需要确认。
...
- `FocusView.swift`：单任务/多任务专注、单项暂停、真实时间结算；启动后联动 Live Activity。
...
**验证**
- iPhone 17 Pro Max 模拟器 `xcodebuild test -scheme Kairos` 通过。
- 测试文件 6 个、用例 42 个：`PlannerTests` 8、`KairosAdvisorTests` 15、`TimeBiasReflectorTests` 9、`GraceMessageGeneratorTests` 4、`LiveActivityManagerTests` 2、`FocusSessionStoreTests` 4。
...
- iPhone 17 Pro Max 模拟器构建成功；`KairosTests` 42 例通过。
- 覆盖：后台时间扣减、杀进程后专注恢复、单项暂停不扣时、当天相邻顺延、Nudge 非评判文案、时间盲症校准排程、体面延误话术、Live Activity 内容状态。
